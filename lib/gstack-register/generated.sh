...
_gstack_register_write_skills_index() {
  local gstack_dir="$1" generated_dir opencode_generated_dir
  local claude_dir codex_dir grok_dir muse_dir gemini_skill_dir opencode_dir
  local dst tmp i skill_dir name link_name
  ...
  claude_dir=$(_gstack_register_claude_skills_dir) || return 1
  codex_dir=$(_gstack_register_codex_skills_dir) || return 1
  grok_dir=$(_gstack_register_grok_skills_dir) || return 1
  muse_dir=$(_gstack_register_muse_skills_dir) || return 1
  gemini_skill_dir=$(_gstack_register_gemini_skills_dir) || return 1
  opencode_dir=$(_gstack_register_opencode_skills_dir) || return 1
  dst="$generated_dir/SKILLS.md"
  mkdir -p "$generated_dir" || return 1
  _gstack_register_load_source_skills "$gstack_dir" || return 1

  # Later scans overwrite earlier ones, exactly like the generation loop
  # overwrites one shared dest when two source skills normalize to one link.
  for i in "${!_GSTACK_REGISTER_SOURCE_SKILL_DIRS[@]}"; do
    ...
    index_sources["$link_name"]="$skill_dir"
  done

  _gstack_register_sibling_tmp_for "$dst" || return 1
  tmp="$REPLY"
  {
    printf '# gstack-register skill provenance\n\n'
    printf '<!-- gstack-register-source: %s/SKILL.md -->\n' "$gstack_dir"
    printf '<!-- gstack-register-generator: %s -->\n' \
      "$_GSTACK_REGISTER_SKILLS_INDEX_VERSION"
    printf "\nThis index is generated by \`gstack-register sync\` from the source"
    printf ' scan; do not edit it.\nEach section maps one agent-visible skill link'
    printf ' to its generated body and the upstream file that body tracks.\n'
    printf 'Agent destinations exist only for installed agents.\n'
    printf 'The umbrella link gstack-gstack is intentionally absent: it is'
    printf ' never generated or linked.\n'
    printf '\n## Re-verifying a misbehaving skill\n\n'
    printf "1. Resolve the agent-visible link: \`readlink <agent-dir>/%s\` must\n" \
      '<link>'
    printf "   print the \`generated\` directory below (without \`/SKILL.md\`).\n"
    printf "2. Confirm the generated body: the \`gstack-register-source\` and\n"
    printf "   \`gstack-register-generator\` markers in the \`generated\` file must\n"
    printf "   match the \`upstream\` and \`generated\` rows of its section.\n"
    printf "3. Repair with \`gstack-register sync\`. A link pointing elsewhere is\n"
    printf '   an unmanaged collision and stays untouched by design; a body whose\n'
    printf '   markers disagree with this index is rewritten.\n'
    while IFS= read -r link_name; do
      [ -n "$link_name" ] || continue
      skill_dir="${index_sources[$link_name]}"
      printf '\n## %s\n' "$link_name"
      printf -- '- claude: %s/%s -> %s/%s\n' \
        "$claude_dir" "$link_name" "$generated_dir" "$link_name"
      printf -- '- codex: %s/%s -> %s/%s\n' \
        "$codex_dir" "$link_name" "$generated_dir" "$link_name"
      printf -- '- gemini: %s/%s -> %s/%s\n' \
        "$gemini_skill_dir" "$link_name" "$generated_dir" "$link_name"
      printf -- '- grok: %s/%s (rewritten copy of %s/%s/SKILL.md)\n' \
        "$grok_dir" "$link_name" "$generated_dir" "$link_name"
      printf -- '- muse: %s/%s (installed copy of %s/%s)\n' \
        "$muse_dir" "$link_name" "$generated_dir" "$link_name"
      printf -- '- opencode: %s/%s -> %s/%s\n' \
        "$opencode_dir" "$link_name" "$opencode_generated_dir" "$link_name"
      printf -- '- generated: %s/%s/SKILL.md (%s)\n' \
        "$generated_dir" "$link_name" "$_GSTACK_REGISTER_GENERATED_SKILL_VERSION"
      printf -- '- upstream: %s/SKILL.md\n' "$skill_dir"
    done < <(printf '%s\n' "${!index_sources[@]}" | LC_ALL=C sort)
  } >"$tmp" || {
    _gstack_register_remove_temp "$tmp" || true
    return 1
  }
...
_gstack_register_write_generated_skills() {
  ...
  _gstack_register_write_skills_index "$gstack_dir" || rc=1
  return "$rc"
}
